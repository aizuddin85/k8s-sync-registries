...
func TestPullAndPushImageWithTLS(t *testing.T) {
    ...
    err := pullAndPushImage(ctx, registry, "nonexistent-tag", "admin", "admin123", sourceCtx)
    assert.Error(t, err)
    assert.Contains(t, err.Error(), "unauthorized: access to the requested resource is not authorized")
}
...
// New version testing cases
func TestSortTagsWithMultipleMinors(t *testing.T) {
    // Create test data with multiple minor versions and patches
    tags := []string{
        "v1.2.0", "v1.2.1", "v1.2.2",  // 1.2.x series
        "v1.3.0", "v1.3.1", "v1.3.2",  // 1.3.x series
        "v1.4.0", "v1.4.1",            // 1.4.x series
        "v2.0.0", "v2.0.1",            // 2.0.x series (should be ignored)
        "v1.2-alpha", "v1.3-beta",      // Should be filtered out
    }
    ...
    tests := []struct {
        ...
        },
        {
            ...
            versionFilters: []VersionRequirement{
                {Major: 1, Minor: 2, GetLatest: true},  // Only latest from 1.2
                {Major: 1, Minor: 3, GetLatest: false}, // All from 1.3
            },
            want: []string{
                "v1.3.2", "v1.3.1", "v1.3.0",
                "v1.2.2",
        ...
        },
        {
            ...
            versionFilters: []VersionRequirement{
                {Major: 1, Minor: 5, GetLatest: true}, // Doesn't exist
                {Major: 1, Minor: 2, GetLatest: true}, // Does exist
            },
            want: []string{"v1.2.2"},
...
func TestSortTagsEdgeCases(t *testing.T) {
    tests := []struct {
        ...
        },
        {
            ...
            versionFilters: []VersionRequirement{},
            want: make([]string, 0),
            description: "Should handle empty filters",
        },
        {
...
func createTestYamlFile(t *testing.T, content []byte) string {
